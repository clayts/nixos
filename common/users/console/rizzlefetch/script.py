...
def big_text(text: str) -> list[str]:
	return subprocess.run(["toilet", "-f" ,"future", "--", text], capture_output=True, text=True, check=True).stdout.split("\n")
...
def merge_frames(left_frames: list[list[str]], right_frames: list[list[str]]) -> list[list[str]]:
...
def hostname() -> str:
	return subprocess.run(["hostname"], capture_output=True, text=True, check=True).stdout.rstrip("\n")
...
def separator() -> list[list[str]]:
	...
	effect.effect_config.final_gradient_stops = (Color("333333"),Color("555555"),)
...
def info_keyframe() -> list[str]:
	return [" " + hardware_name(), " " +  distro_name(), " " +  kernel_name(),]
